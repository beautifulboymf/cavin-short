...
\begin{table}[t]
\caption{Ablation at $\rho=30$ (PEHE $\downarrow$, mean$^{\pm\text{std}}$, 5 seeds). \textbf{Bold} = best per row.}
\label{tab:var}
\centering
\footnotesize
...
For GDC, GPE alone \emph{hurts} on CoraFull ($4.64 \to 5.14$) because the positional proxy amplifies noise---but the combination yields the best PEHE on CoraFull and DBLP (up to 20\% reduction). On PubMed, variance weighting alone achieves a 29\% reduction, suggesting that when heteroscedasticity dominates confounding, noise suppression is the primary driver. TARNet shows the same complementarity pattern on DBLP ($6.78 \to 5.56$, $-18\%$ with the combination), though on CoraFull variance weighting alone edges ahead. The optimal configuration is model- and dataset-dependent, but the combination is never far from the best single module and often dominates---consistent with the dual-root prediction. On real-world tabular benchmarks, variance weighting yields a 4$\times$ Qini improvement on continuous outcomes (Hillstrom spend; full results in supplementary).
